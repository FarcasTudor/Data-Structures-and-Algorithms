...
void testSterge() {
	...
	for (int i = -100; i < 100; i++) {
		if (i % 2 == 0) {
			//cout << i << " are aparitiile: "<< c.nrAparitii(i) << endl;
			assert(c.nrAparitii(i) == 0);
			assert(c.cauta(i) == false);
			assert(c.sterge(i) == false);
		}
...
void testQuantity() {
	...
	assert(c.dim() == count);
	cout << "Quantity tests passed!" << endl;
}
...
void testCerintaLAB()
{
	...
	c.adauga(100);
	c.adauga(100);
	int verif = c.elementeCuFrecventaData(2);
	//cout << verif << endl;
	assert(verif == 2);
	verif = c.elementeCuFrecventaData(1);
	//cout << verif << endl;
	assert(verif == 2);
	verif = c.elementeCuFrecventaData(3);
	assert(verif == 1);
	cout << "Cerinta LAB test paseed!" << endl;
}
