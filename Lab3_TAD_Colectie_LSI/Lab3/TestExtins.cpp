...
void testQuantity() {//scopul e sa adaugam multe date 
	Colectie c;
	for (int i = 10; i >= 1; i--) {
		for (int j = -8000; j < 8000; j = j + i) {
			c.adauga(j);
		}
	}
	assert(c.dim() == 46865);

	assert(c.nrAparitii(-30000) == 0);
...
void testadaugaAparitiiMultiple()
{
	...
	assert(c.vida() == false);
	assert(c.dim() == 10);
	c.adaugaAparitiiMultiple(5, 1);
	assert(c.nrAparitii(1) == 6); // verificam daca am modificat corect numarul de aparitii al elementului 1
	c.adaugaAparitiiMultiple(100, 2);
	assert(c.nrAparitii(2) == 101);
	c.adaugaAparitiiMultiple(10,10); //cazul in care elementul 10 nu exista
	assert(c.nrAparitii(10) == 10);
